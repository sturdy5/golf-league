...
if (isset($_GET["code"]) && strlen($_GET["code"])) {
	...
	$result = mysqli_query($query) or die ("Error retrieving the record corresponding to the activation code");
	if (mysqli_num_rows($result)) {
		$user = mysqli_fetch_array($result);
		$query = "update users set active = 1 where id = ".$user["userid"];
		$result = mysqli_query($query) or die ("Error activating the user");
		$query = "delete from activation where code = '$lookup' LIMIT 1";
		$result = mysqli_query($query) or die ("Error removing the activation record");
